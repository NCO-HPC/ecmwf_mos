...
echo MDLLOG: `date` - Begin execmmos_extprep.sh.ecf 47km

Ref_date=`echo $PDY | cut -c 5-8`$cyc
PDYcycz=$PDY$cyc

#######################################################################
# LOOP THROUGH PROJECTIONS (102 to 240 hours at 6-hour intervals)
#######################################################################
for tau in $(seq 102 6 240)
do
#######################################################################
...
export FORT27="mdl_ecmxpkd.$cycle"
export FORT28="$FIXmdl/mdl_ecmxprep47_grbtomdlp.lst"
...
chgrp rstprod mdl_ecmxpkd.$cycle
chmod 640 mdl_ecmxpkd.$cycle
#######################################################################
# COPY FILES TO COM
#######################################################################
...
if test $SENDCOM = 'YES'
then
  cpreq --preserve=mode,ownership mdl_ecmxpkd.$cycle $COMOUT/mdl_ecmxpkd.$cycle
  cpreq --preserve=mode,ownership mdl_xpcp+pcp.$cycle.pgrb $COMOUT/mdl_xpcp+pcpx.$cycle.pgrb
  cpreq --preserve=mode,ownership mdl_xpcp+pcp.$cycle.pgrbi $COMOUT/mdl_xpcp+pcpx.$cycle.pgrbi
fi
...
postmsg "$jlogfile" "$msg"
echo MDLLOG: `date` - Job execmmos_extprep.sh.ecf 47km has ended.
